...
public class CoreConfigSingboxServiceTests
{
    ...
    [Fact]
    public void GenerateClientConfigContent_Tun_ShouldProtectCoreTrafficBeforeGeneralDnsHijack()
    {
        var config = CoreConfigTestFactory.CreateConfig(ECoreType.sing_box);
        ...
        result.Success.Should().BeTrue($"ret msg: {result.Msg}");
        var cfg = JsonUtils.Deserialize<SingboxConfig>(result.Data!.ToString())!;
        var generalDnsRuleIndex = cfg.route.rules.FindIndex(rule =>
            rule.action == "hijack-dns" && rule.type == "logical");
        var coreDirectRuleIndex = cfg.route.rules.FindIndex(rule =>
            rule.outbound == Global.DirectTag
            && rule.process_name?.Contains(Utils.GetExeName("sing-box")) == true);

        generalDnsRuleIndex.Should().BeGreaterThan(-1);
        coreDirectRuleIndex.Should().BeGreaterThan(-1);
        coreDirectRuleIndex.Should().BeLessThan(generalDnsRuleIndex);
    }
