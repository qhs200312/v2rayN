...
public class StatisticsManager
{
    ...
    public async Task Init(Config config, Func<ServerSpeedItem, Task> updateFunc, bool forceRealtimeSpeed = false)
    {
        _config = config;
        _updateFunc = updateFunc;
        if (config.GuiItem.EnableStatistics || _config.GuiItem.DisplayRealTimeSpeed || forceRealtimeSpeed)
        {
            await InitData();
